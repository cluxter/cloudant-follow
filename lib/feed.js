...
Feed.prototype.follow = function follow_feed() {
  ...
  if ([ 'continuous', 'live', 'longpoll', 'normal' ].indexOf(self.feed) === -1) {
    throw new Error(`Invalid feed option: ${self.feed}`);
  }

  if (self.feed === 'live') {
    self.feed = 'continuous'; // alias
  }
...
Feed.prototype.on_couch_data = function on_couch_data(change) {
  ...
  if ('last_seq' in change) {
    var stopFeed = false;
    if (change.pending === undefined) {
      stopFeed = true;
    } else if (change.pending === 0) {
      stopFeed = true;
    } else if (self.query_params.limit > 0) {
      stopFeed = true;
    }
